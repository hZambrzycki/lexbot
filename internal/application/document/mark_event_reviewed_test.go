...
func TestMarkEventReviewed_UpdatesReviewState(t *testing.T) {
	...
	cf, err := domaincasefile.NewCaseFile(ids.NewID(), clientID, "Expediente review")
	if err != nil {
		t.Fatalf("casefile.NewCaseFile: %v", err)
	}
	cf.Reference = "EXP-REVIEW-001"
	cf.Type = domaincasefile.TypeCivil
	cf.Description = "Test mark reviewed"
	...
	doc, err := domaindocument.NewDocument(
		...
		cf.ID,
		"review.txt",
		filepath.Join(".lexbox", "storage", "documents", "review.txt"),
	)
	if err != nil {
		t.Fatalf("document.NewDocument: %v", err)
	}
	doc = doc.WithUpdatedMetadata("text/plain", "hash-review-test")
	...
	uc := MarkEventReviewed{
		Events: eventRepo,
	}

	result, err := uc.Execute(ctx, MarkEventReviewedInput{
		EventID: event.ID.String(),
	})
	if err != nil {
		t.Fatalf("MarkEventReviewed.Execute: %v", err)
	}
	...
	if result.ReviewStatus != domaindocument.ReviewStatusReviewed {
		t.Fatalf("expected review status %q, got %q", domaindocument.ReviewStatusReviewed, result.ReviewStatus)
	}
	...
	if saved.ReviewStatus != domaindocument.ReviewStatusReviewed {
		t.Fatalf("expected persisted review status %q, got %q", domaindocument.ReviewStatusReviewed, saved.ReviewStatus)
	}
